...
app.get("/home", (req, res) => {
  if (req.session.uid) {
    let msg = req.session.msg || "";
    const sql = "SELECT tweet.*, user.username FROM tweet INNER JOIN user ON tweet.uid = user.uid WHERE tweet.uid = ? OR tweet.content LIKE CONCAT('%', ?, '%') OR tweet.uid IN (SELECT follow_uid FROM followers WHERE follow_uid = ?) ORDER BY tweet.datetime DESC";
